...
function abort(msg: any) {
    console.error(msg || 'Error occured');
    process.exit(1);
}
function readFileCallback(path: any) {
...
function stripBasePath(path: any) {
...
if (program.standardJson) {
    var input = fs.readFileSync((process.stdin as any).fd).toString('utf8');
