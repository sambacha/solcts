...
function runTests(solc: any, versionText: any) {
    console.log(`Running tests with ${versionText} ${solc.version()}`);
    function getBytecode(output: any, fileName: any, contractName: any) {
        try {
            var outputContract;
    ...
    function getBytecodeStandard(output: any, fileName: any, contractName: any) {
        try {
            var outputFile;
            if (semver.lt(solc.semver(), '0.4.9')) {
    ...
    function getGasEstimate(output: any, fileName: any, contractName: any) {
        try {
            var outputFile;
            if (semver.lt(solc.semver(), '0.4.9')) {
                outputFile = output.contracts[''];
    ...
    function expectError(output: any, errorType: any, message: any) {
        if (output.errors) {
            for (var error in output.errors) {
                ...
                if ((error as any).type === errorType) {
                    if (message) {
                        if ((error as any).message.match(message) !== null) {
                            return true;
                        }
                    }
                    else {
    ...
    function expectNoError(output: any) {
        if (output.errors) {
            for (var error in output.errors) {
                error = output.errors[error];
                if ((error as any).severity === 'error') {
                    return false;
                }
            }
        }
    ...
    tape(versionText, function (t: any) {
        var tape = t.test;
        tape('Version and license', function (t: any) {
            t.test('check version', function (st: any) {
                st.equal(typeof solc.version(), 'string');
                st.end();
            });
            t.test('check semver', function (st: any) {
                st.equal(typeof solc.semver(), 'string');
                st.end();
            });
            t.test('check license', function (st: any) {
                st.ok(typeof solc.license() === 'undefined' || typeof solc.license() === 'string');
                st.end();
            });
        });
        tape('Compilation', function (t: any) {
            t.test('single files can be compiled (using lowlevel API)', function (st: any) {
            ...
            t.test('invalid source code fails properly (using lowlevel API)', function (st: any) {
            ...
            t.test('multiple files can be compiled (using lowlevel API)', function (st: any) {
            ...
            t.test('lazy-loading callback works (using lowlevel API)', function (st: any) {
            ...
            t.test('lazy-loading callback works (with file not found) (using lowlevel API)', function (st: any) {
                // <0.2.1 doesn't have this
                if (typeof solc.lowlevel.compileCallback !== 'function') {
                    st.skip('Low-level compileCallback interface not implemented by this compiler version.');
                    st.end();
            ...
            t.test('lazy-loading callback works (with exception) (using lowlevel API)', function (st: any) {
                // <0.2.1 doesn't have this
                if (typeof solc.lowlevel.compileCallback !== 'function') {
                    st.skip('Low-level compileCallback interface not implemented by this compiler version.');
                    st.end();
            ...
            t.test('lazy-loading callback fails properly (with invalid callback) (using lowlevel API)', function (st: any) {
                // <0.2.1 doesn't have this
                if (typeof solc.lowlevel.compileCallback !== 'function') {
                    st.skip('Low-level compileCallback interface not implemented by this compiler version.');
                    st.end();
            ...
            t.test('file import without lazy-loading callback fails properly (using lowlevel API)', function (st: any) {
                // <0.2.1 doesn't have this
                if (typeof solc.lowlevel.compileCallback !== 'function') {
                    st.skip('Low-level compileCallback interface not implemented by this compiler version.');
                    st.end();
            ...
            t.test('compiling standard JSON (using lowlevel API)', function (st: any) {
                ...
                function bytecodeExists(output: any, fileName: any, contractName: any) {
                    try {
            ...
            t.test('invalid source code fails properly with standard JSON (using lowlevel API)', function (st: any) {
                // TODO: try finding an example which doesn't crash it?
                if (semver.eq(solc.semver(), '0.4.11')) {
                    st.skip('Skipping on broken compiler version');
                    st.end();
            ...
            t.test('compiling standard JSON (with callback) (using lowlevel API)', function (st: any) {
                if (typeof solc.lowlevel.compileStandard !== 'function') {
                    st.skip('Low-level compileStandard interface not implemented by this compiler version.');
                    st.end();
                    return;
            ...
            t.test('compiling standard JSON (single file)', function (st: any) {
                var input = {
                    'language': 'Solidity',
                    'settings': {
                        'outputSelection': {
            ...
            t.test('compiling standard JSON (multiple files)', function (st: any) {
                // <0.1.6 doesn't have this
            ...
            t.test('compiling standard JSON (abstract contract)', function (st: any) {
                // <0.1.6 doesn't have this
                if (!solc.features.multipleInputs) {
                    st.skip('Not supported by solc');
                    st.end();
            ...
            t.test('compiling standard JSON (with imports)', function (st: any) {
                // <0.2.1 doesn't have this
            ...
            t.test('compiling standard JSON (using libraries)', function (st: any) {
            ...
            t.test('compiling standard JSON (with warning >=0.4.0)', function (st: any) {
            ...
            t.test('compiling standard JSON (using libraries) (using lowlevel API)', function (st: any) {
                // 0.4.0 has a bug with libraries
                if (semver.eq(solc.semver(), '0.4.0')) {
                    st.skip('Skipping on broken compiler version');
                    st.end();
            ...
            t.test('compiling standard JSON (invalid JSON)', function (st: any) {
            ...
            t.test('compiling standard JSON (invalid language)', function (st: any) {
            ...
            t.test('compiling standard JSON (no sources)', function (st: any) {
                var output = JSON.parse(solc.compile('{"language":"Solidity"}'));
                st.ok(expectError(output, 'JSONError', 'No input sources specified.'));
                st.end();
            });
            t.test('compiling standard JSON (multiple sources on old compiler)', function (st: any) {
    ...
    if (versionText === 'latest') {
        tape('Loading Legacy Versions', function (t: any) {
            t.test('loading remote version - development snapshot', function (st: any) {
                // getting the development snapshot
                st.plan(2);
                solc.loadRemoteVersion('latest', function (err: any, solcSnapshot: any) {
                    if (err) {
                        st.plan(1);
